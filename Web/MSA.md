...
* 여기서 `Monolithic Architecture`란, 소프트웨어의 모든 구성요소가 한 프로젝트에 모여있는 방식이다.
...
* 장점
    ...
    * 유연한 기술을 적용 할 수 있다. 예를 들면 채팅서버는 node.js를 사용하고 어떤 서버는 django를 사용하는 방식으로 각 서버의 언어나 프레임워크를 유연하게 선택할 수 있다.
...
# 🔍MSA 대표적인 적용 사례
MSA의 대표적인 사례로는 우버가 있다. 우버는 원래 `Monolithic Architecture`로 만들어졌다. 처음에는 하나의 프로젝트로 개발하는 것이 편했고 이슈를 처리하는데에도 문제가 없었지만 점점 프로젝트의 규모가 커지면서 확장성과 지속적인 연동 문제에 직면하게 됐다.
